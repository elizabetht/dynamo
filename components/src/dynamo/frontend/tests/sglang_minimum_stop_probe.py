...
"""CPU HTTP minimum-token regression with scripted tokens; no model weights or GPU."""
...
async def main(output, expect_baseline_failure):
    ...
    text = "Hi STOP one two three four five six seven eight"
    cases = [
        ("early-string", text + " STOP tail", 8, text + " ", "stop"),
        ("ordinary-string", "Hi STOP tail", 0, "Hi ", "stop"),
        ("minimum-length", text, 8, text, "length"),
    ]
    ...
    with tempfile.TemporaryDirectory(prefix="minimum-stop-", dir=output) as discovery:
        ...
        tasks = []
        try:
            namespace = "minimum-stop-" + uuid.uuid4().hex
            ...
            async with aiohttp.ClientSession(
                ...
                for name, text, minimum, expected, finish in cases:
                    for batch in [1, 7]:
                        ...
                        for interval in [1, 20]:
                            ...
                            for streaming in [True, False]:
                                ...
                                assert (
                                    calls[-1]["stop_conditions"]["min_tokens"]
                                    == minimum
                                )
    ...
    assert len(rows) == 24
    print(f"{len(rows) - len(failed)}/24 CPU HTTP cases passed")
    if expect_baseline_failure:
        assert len(failed) == 16 and all(
            row["case"] != "ordinary-string" for row in failed
        )
        assert all(
            row["content"] == "Hi " and row["finishes"] == ["stop"] for row in failed
        )
    else:
        assert not failed, [(row["case"], row["content"]) for row in failed]
...
if __name__ == "__main__":
    ...
    asyncio.run(
        asyncio.wait_for(main(args.output.resolve(), args.expect_baseline_failure), 150)
    )
