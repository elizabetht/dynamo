...
"""Replay JSON-array call completeness against a separately managed real server."""
import argparse
import asyncio
import json
        ...
        for case in json.loads(args.corpus.read_text()):
            for iteration in range(args.repetitions):
                ...
                try:
                    ...
                    validated = validate(payload, events)
                    expected = (
                        2
                        if args.arm == "candidate"
                        and payload.get("parallel_tool_calls") is True
                        else 1
                    )
                    assert len(validated["calls"]) == expected, validated
                    row.update(validated, valid=True)
