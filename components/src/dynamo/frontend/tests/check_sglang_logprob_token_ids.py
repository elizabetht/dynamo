...
"""Check SGLang logprob formatting through CPU HTTP with scripted worker tokens."""
...
async def main(output, expect_baseline):
    ...
    with tempfile.TemporaryDirectory(
        ...
        tasks = []
        try:
            namespace = "logprob-ids-" + uuid.uuid4().hex
            ...
            async with aiohttp.ClientSession(
                ...
                for (
                    ...
                    if not extra["return_tokens_as_token_ids"]:
    ...
    id_rows = [row for row in rows if row["id_format_correct"] is not None]
    assert len(id_rows) == 32
    assert all(row["id_format_correct"] != expect_baseline for row in id_rows)
    for row in rows:
        if row["id_format_correct"]:
            assert all(
                entry["bytes"] == list(entry["token"].encode("utf-8"))
                for entry in row["logprob_entries"]
            )
        assert all(
            not entry["top_logprobs"] and "_token_id" not in entry
            for entry in row["logprob_entries"]
        )
...
if __name__ == "__main__":
    ...
    asyncio.run(asyncio.wait_for(main(args.output, args.expect_baseline), 240))
