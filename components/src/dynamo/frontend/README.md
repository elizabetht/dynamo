...
requests using only nested template settings retain their previous behavior.
...
values of the two top-level flags.
...
pass and the worker receives the exact expected tokens. All ten new unit cases pass.
The broader processor suite reports 301 passed and one setup error because the offline
TinyLlama byte-fallback tokenizer fixture is not cached.
...
PYTHONPATH=components/src python -m pytest \
  ...
  -k test_chat_generation_controls -q
...
runtime. Native logs still report nine tasks at interpreter exit; complete native shutdown
